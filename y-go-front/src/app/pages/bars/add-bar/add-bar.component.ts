...
export class AddBarComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.barForm = this.fb.group({
      ...
      description: [``, [Validators.required, Validators.maxLength(150), Validators.minLength(15)]],
    ...
    if (this.barForm.valid) {
      ...
      this.barService.addBar(barData).subscribe({
        next: (barResponse: BarModel) => {
          ...
          this.snackbarService.openSnackBar(`Le bar ${barResponse.name} a bien été enregistré`, `Fermer`)
        },
        error: (error) => {
          console.error(`Erreur lors de l'enregistrement du bar:`, error)
          this.barForm.setErrors({
            submit: error.error.message || `Une erreur s'est produite lors de l'enregistrement du bar.`,
          })
        },
      })
    }
  ...
  onAddressChange(): void {
    if (this.barForm.value.adresse.length > 5) {
      this.geocodingService.getCoordinates(this.barForm.value.adresse).subscribe((data) => {
