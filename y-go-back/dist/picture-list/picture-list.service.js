...
let PictureListService = exports.PictureListService = class PictureListService {
    constructor(pictureListRepository) {
        this.pictureListRepository = pictureListRepository;
    }
    async create(createPictureListDto) {
        try {
            const pictureList = this.pictureListRepository.create(createPictureListDto);
            const savedPictureList = await this.pictureListRepository.save(pictureList);
    ...
    async findOne(id) {
        const responsePictureList = await this.pictureListRepository.findOne({ where: { id } });
    ...
    async update(id, updatePictureListDto) {
        ...
        const updatedPictureList = await this.pictureListRepository.findOne({ where: { id } });
...
exports.PictureListService = PictureListService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(picture_list_entity_1.PictureList)),
