...
let PictureListController =
  (exports.PictureListController = class PictureListController {
    constructor(pictureListService) {
      this.pictureListService = pictureListService;
    }
    async create(createPictureListDto) {
      const barList =
        await this.pictureListService.create(createPictureListDto);
      return barList;
    }
    async uploadFiles(pictureListId, files) {
      const filePaths = files.map((file) => file.path);
      await this.pictureListService.update(pictureListId, {
        pictures: filePaths,
      });
      return {
    ...
    async findOne(id) {
      ...
      if (!pictureListSelected) {
        throw new common_1.NotFoundException(
          `PictureList with id ${id} not found`,
        );
      }
    ...
    async update(id, updatePictureListDto) {
      const pictureListUpdated = await this.pictureListService.update(
        id,
        updatePictureListDto,
      );
...
__decorate(
  [
    ...
    __metadata('design:paramtypes', [
      create_picture_list_dto_1.CreatePictureListDto,
    ]),
    __metadata('design:returntype', Promise),
  ],
  PictureListController.prototype,
  'create',
  null,
);
__decorate(
  [
    ...
    __param(
      1,
      (0, common_1.UploadedFiles)(
        new FileSizeValidationPipe_1.FileSizeValidationPipe(),
      ),
    ),
    __metadata('design:type', Function),
...
__decorate(
  [
    ...
    __metadata('design:paramtypes', [
      String,
      update_picture_list_dto_1.UpdatePictureListDto,
    ]),
    __metadata('design:returntype', Promise),
  ],
  PictureListController.prototype,
