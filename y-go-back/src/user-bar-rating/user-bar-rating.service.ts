...
export class UserBarRatingService {
  ...
  async create(
    ...
    try {
      const user = await this.usersServices.findOne(
        createUserBarRatingDto.user.id,
      );
      const bar = await this.barsService.findOnePartial(
        createUserBarRatingDto.bar.id,
      );
      if (!user) {
        throw new NotFoundException(
          `User with ID ${createUserBarRatingDto.user.id} not found`,
        );
      }
      if (!bar) {
        throw new NotFoundException(
          `Bar with ID ${createUserBarRatingDto.bar.id} not found`,
        );
      }
  ...
  async findAllRatesByBarId(barId: string): Promise<ResponseRateDto[]> {
  ...
  async update(
    ...
    const updatedRate: UserBarRating = await this.rateRepository.findOne({
      where: { id },
    });
  ...
  async countVoters(barId: string): Promise<number> {
    const ratings = await this.rateRepository.find({
      where: { bar: { id: barId } },
    });
    console.log(ratings);
