...
export class PictureListController {
  ...
  @Post()
  async create(
    @Body() createPictureListDto: CreatePictureListDto,
  ): Promise<CreatePictureListDto> {
    const barList: ResponsePictureListDto =
  ...
  async findOne(@Param(`id`) id: string): Promise<ResponsePictureListDto> {
    const pictureListSelected: ResponsePictureListDto =
      await this.pictureListService.findOne(id);
  ...
  async findAll(): Promise<ResponsePictureListDto[]> {
    const pictureList: ResponsePictureListDto[] =
      await this.pictureListService.findAll();
    if (!pictureList) {
  ...
  async update(
    @Param(`id`) id: string,
    @Body() updatePictureListDto: UpdatePictureListDto,
  ): Promise<ResponsePictureListDto> {
    const pictureListUpdated: ResponsePictureListDto =
      await this.pictureListService.update(id, updatePictureListDto);
